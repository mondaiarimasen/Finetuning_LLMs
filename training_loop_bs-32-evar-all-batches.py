...
for key in tokenized_datasets_pt.keys():
    print(f"\ncreating dataloader for {key}")
    dataloader[key] = DataLoader(tokenized_datasets_pt[key], batch_size=batch_size, shuffle=True)
    # checking number of batches for key = len(tokenized_datasets['key']) / batch_size
    print(f"checking expected number of {key} batches")
    if len(dataloader[key]) == math.ceil(len(tokenized_datasets_pt[key]) / batch_size):
        print(f"expected number of {key} batches is correct")
    else:
        print(f"expected number of {key} batches is not correct")
        sys.exit()
...
#validation_dataset = tokenized_datasets["validation"].to_dict()
#validation_dataset = {k: torch.tensor(v) for k,v in validation_dataset.items()}
